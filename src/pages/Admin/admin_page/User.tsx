      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Manage system users, administrators and their permissions
          </p>
        </div>

        {/* Nút 'Invite New User' co dãn theo màn hình */}
        <button className="flex items-center justify-center gap-2 bg-primary hover:bg-primary/90 text-white px-4 py-2.5 rounded-xl font-semibold transition-all shadow-lg shadow-primary/25 w-full sm:w-auto shrink-0">
          <Plus size={20} />
              ...
              {users.map((user) => (
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="font-semibold text-slate-900 dark:text-white uppercase text-sm truncate">
                          {user.name}
                        </p>
                  ...
                  <td className="px-6 py-4">
                    ...
                    <div
                      className={`inline-flex items-center gap-1.5 font-bold text-xs uppercase ${user.role === "Super Admin" ? "text-primary" : "text-slate-600 dark:text-slate-400"} whitespace-nowrap`}
                    >
                      <ShieldCheck size={16} />
                      <span>{user.role}</span>
                    </div>
