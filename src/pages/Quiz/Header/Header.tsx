...
function Header({currentIndex, dataQuestion,topicId, handleSubmit} : HeaderProps){
    const navigate = useNavigate()
    return <header className="sticky top-0 z-50 w-full border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" className="rounded-lg" onClick={() => navigate("/home") }>
         ...
         <CounDownTimer initialTime={10} topicId={topicId || null} handleSubmit={handleSubmit}/>
        </div>
        {/* Mobile Progress */}
